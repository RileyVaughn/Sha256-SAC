...
def  SubplotThreshByLevel():
    ...
    names = ["Majority","Choose","Integer Addition","K Constants", "Message Scheduler","Σ0","Σ1"]
    ...
    for i, ax in enumerate(axs_flat):
        if i < 7:
            ...
            #ax.plot(x_grey,y_grey,marker='o',linestyle='',color='grey',markersize=ms)
            ax.plot(x_color,y_color,marker='o',linestyle='',color=colors[i],markersize=ms)
            ax.plot(x_grey,y_grey,marker='o',linestyle='',color='grey',markersize=ms)
            ax.set_title(f"Function: {names[i]}")
